...
namespace log4cpp {
    ...
    struct FullFilenameComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
			out << event.file;
        }
    };
    ...
    struct FilenameComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
			// remove path in file name
			std::string::size_type pos = event.file.find_last_of( "\\/" );
			if ( pos == std::string::npos )
				out << event.file;
			else
				out << event.file.substr( pos + 1 );
        }
    };
    ...
    void PatternLayout::setConversionPattern(const std::string& conversionPattern) throw() {
#ifdef LOG4CPP_HAVE_SSTREAM 
        ...
        while (conversionStream.get(ch)) {
            if (ch == '%') {
                ...
                case 'p':
                    ...
                    break;
                case 'F':
                    component = new FullFilenameComponent();
                    break;
                case 'f':
