...
namespace log4cpp {
    ...
    class LOG4CPP_EXPORT Category {
        ...
        */       
         ...
         **/
        bool isPriorityEnabled(Priority::Value priority) const throw() {
         ...
         * @return the parent category.
         **/
        const Category* getParent() const throw() {
			return _parent;
		}
		 ...
		 * @param file filename where the log message comes from
         ...
         * @param ... The arguments for stringFormat 
         **/  
        void trace(const char* stringFormat, ...) throw();
		...
		/**
		 * Add a category to the list of children
		 */
		void _addChild(Category* child) throw();

		/**
		 * Propagate a change in the parent priority.
		 */
		void _updateChainedPriority(Priority::Value priority) throw();
		...
		typedef std::vector< Category* > ChildrenList;
	    mutable threading::Mutex _childrenListMutex;
	
		/**
		 * The list of all children in the hierarchy.
		 */
		...
		/**
		 * The effective priority as determined by the hierarchy.
		 */
        volatile Priority::Value _chainedPriority;
...
#ifndef LOG4CPP_DISABLE_LOGGING

// Macro version of Category::log for more efficiency and line numbers. 
#define LOG4CPP_LOG( logger, priority, message, file, line ) \
{ \
	if ( (logger).isPriorityEnabled( priority ) ) \
	{ \
		std::ostringstream _l_s; _l_s << message; \
		(logger).log( priority, _l_s.str(), file, line ); \
	} \
}
...
/** 
 * Macro version of Category::trace for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_TRACE( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::TRACE, message, __FILE__, __LINE__ )

/** 
 * Macro version of Category::debug for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_DEBUG( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::debug, message, __FILE__, __LINE__ )

/** 
 * Macro version of Category::info for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_INFO( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::INFO, message, __FILE__, __LINE__ )

/** 
 * Macro version of Category::notice for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_NOTICE( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::NOTICE, message, __FILE__, __LINE__ )

/** 
 * Macro version of Category::warn for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_WARN( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::WARN, message, __FILE__, __LINE__ )

/** 
 * Macro version of Category::error for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_ERROR( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::error, message, __FILE__, __LINE__ )

/** 
 * Macro version of Category::crit for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_CRIT( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::CRIT, message, __FILE__, __LINE__ )

/** 
 * Macro version of Category::alert for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_ALERT( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::ALERT, message, __FILE__, __LINE__ )

/** 
 * Macro version of Category::fatal for more efficiency and line numbers. 
 * @param logger reference to a Category object
 * @param message the log message. Multiple parameters can be concatenated using the stream << operator.
 */
#define LOG4CPP_FATAL( logger, message ) LOG4CPP_LOG( logger, log4cpp::Priority::FATAL, message, __FILE__, __LINE__ )
